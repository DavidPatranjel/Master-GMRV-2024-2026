...
vec3 ComputePhongIllumination(vec3 light_position, vec3 culoareLumina)
{
    ...
    vec3 diffuse_component = material_kd * culoareLumina * max(dot(N,L), 0);
    ...
    if (length(diffuse_component) > 0)
        specular_component = material_ks * culoareLumina * pow(max(dot(V, R), 0), material_shininess);
...
float ComputeDistanceAttenuation(vec3 light_position, vec3 point_position)
{
    ...
    float d = length(point_position - light_position);
    return 1 / (1 + d * d);
}
...
vec3 ComputeSpotLightSourcesIllumination()
{
    ...
    for (int i=0;i<10;i++) {
        ...
        if (cos_theta_angle > cos_phi_angle)
        {
            ...
            float d  = ComputeDistanceAttenuation(light_position, world_position);
            ...
            lights_illumination += quadratic_spot_light_att_factor * light_illumination;
        }
    }
